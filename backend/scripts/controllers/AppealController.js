angular.module('e-homework').controller('AppealController', function ($scope, $compile, $cookies, $filter, $state, $routeParams, HTTPService, IndexOverlayFactory) {
    ...
    $scope.exportReport = function(DataList){
       console.log(DataList);
        ...
        IndexOverlayFactory.overlayShow();
        HTTPService.clientRequest('appeal/report', params).then(function(result){
        if (result.data.STATUS == 'OK') {
    ...
    $scope.callback = function (Data,AttachFile) {
        var params = {'Data': Data,'AttachFile': AttachFile};

        HTTPService.uploadRequest('appeal/callback', params).then(function (result) {
