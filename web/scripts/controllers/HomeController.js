angular.module('e-homework').controller('HomeController', function ($scope, $cookies, $filter, $state, $sce, $uibModal, HTTPService, IndexOverlayFactory) {
    ...
    $scope.loadSlide = function (action) {
        HTTPService.clientRequest(action, null).then(function (result) {
            ...
            $scope.control = result.data.DATA.BannerControl;
            IndexOverlayFactory.overlayHide();
...
// Thumbnail image controls
    ...
    function showSlides(index) {
        console.log($scope.control.delay);
        if (varSetTimeout) {
            // Call clearTimeout() on hover()
        ...
        if (slides.length !== 0) {
            ...
            if (slides.length > 1) {
                ...
                if (isNaN(video_src.duration)) {
                    if ($scope.control.type == 'N' && count >= slides.length) {

                    } else {
                        varSetTimeout = setTimeout(showSlides, $scope.control.delay*1000); // Change image every 2 seconds
                        count++;
                    }
                    ...
                    varSetTimeout = setTimeout(showSlides, (video_src.duration * $scope.control.delay) + 200);
