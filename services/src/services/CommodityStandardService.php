<?php
    ...
    class CommodityStandardService {
        ...
        public static function searchCommodityStandard($keyword,$dateFrom,$dateTo){
            // $totalPage = ceil($totalRows / $limitRowPerPage);
            return CommodityStandard::where(function($query) use ($keyword,$dateFrom,$dateTo){
                            //$query->where('step', '9');
                            ...
                            if(!empty($dateFrom)){
                                $query->where('useDate', '>', $dateFrom);
                                
                            }
                            if(!empty($dateTo)){
                                $query->where('cancelledDate', '<', $dateTo);
