...
//        
        ...
        public function advsearch($request, $response, $args){
            try{
...
//        
                ...
                $keyword = $params['obj']['keyword']; 
                $dateFrom = $params['obj']['dateFrom']; 
                $dateTo = $params['obj']['dateTo']; 
                
                $SearchResult = [];
                $CommodityStandardResult = CommodityStandardService::searchCommodityStandard($keyword,$dateFrom,$dateTo);
                foreach ($CommodityStandardResult as $key => $value) {
                    $value['title_th'] = $value['standardNameThai'];
                    $value['url'] = '#/standard-commodity/search';
                    array_push($SearchResult, $value);
                ...
                $NewsResult = NewsService::searchNews($keyword,$dateFrom,$dateTo);
                foreach ($NewsResult as $key => $value) {
                    $value['title_th'] = $value['project_th'];
                    $value['url'] = '#/news/detail/' . $value['id'];
                    array_push($SearchResult, $value);
